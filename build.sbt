...
lazy val commonSettings =
  Seq(
    ...
    version := "0.5",
    crossScalaVersions := Seq("2.13.13", "2.12.19", "2.11.12"),
    ...
    libraryDependencies ++= Seq("base", "controls", "fxml", "graphics", "media", "swing", "web").map(
      m => "org.openjfx" % s"javafx-$m" % "25-ea+5" % "provided" classifier osName),
...
lazy val demo = Project("scalafxml-demo-sfx8", file("demo"))
  ...
  .settings(
    ...
    libraryDependencies ++= Seq("base", "controls", "fxml", "graphics", "media", "swing", "web").map(
      m => "org.openjfx" % s"javafx-$m" % "25-ea+5" classifier osName)
  )
